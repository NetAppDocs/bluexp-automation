...
[.lead]
You can access the Cloud Manager web user interface to retrieve several identifiers that are needed with the workflows.
...
Each Connector is assigned a unique identifier. You can retrieve the ID for a Connector and include it in the x-agent-id HTTP request header on a REST API call.

.About this task

The steps needed to obtain the unique identifier for a Connector are presented below. You can optionally derive the client identifier from the Connector ID.
