...
Most of the variable tokens, identifiers, and other variables used in the sample REST API calls consist of long strings of letters, numbers, and special characters. They are considered _opaque_ with no easily discernible content or meaning. Therefore, rather than including the actual original strings, smaller reserved keywords are used instead.

For example, <ACCESS_TOKEN> is a shorthand notation for a temporary string which establishes identity and access based on the OAuth2 standard.
