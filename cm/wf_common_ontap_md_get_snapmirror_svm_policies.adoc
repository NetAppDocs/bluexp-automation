...
Perform the workflow link:wf_aws_cloud_1n_get_wes_.html[Get working environments] and choose the `publicId` values of the required source and destination path parameter. Choose the `svmName` of the destination working environment for the  `destinationSvmName` path parameter.
