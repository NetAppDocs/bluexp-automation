...
The *client identifier* is used to route each API request to the right location within the Cloud Manager platform. You can retrieve the Connector ID which contains the client ID. You can use the Connector ID as the `x-agent-id` HTTP request header in the API workflows.
//You can retrieve the client ID and use it with the `x-agent-id` HTTP request header.

.About this task

The client ID is unique for each Cloud Manager Connector and this value can be obtained from the Connector ID.
