...
In addition, some workflows are not tied to a specific cloud provider and can be performed in any cloud environment. These workflows are considered to be *common* to all the cloud providers. Review the link:wf_common.html[Before you begin] page for more information on each of the Common workflow categories.
...
A working environment is the context within which a Cloud Volumes ONTAP instance is deployed and run. These workflows allow you to create working environments and perform related administration. You can perform specific workflows to create a working environment based on your preferred licensing instance (PAYGO or BYOL), remove a working environment, or retrieve working environment details. In addition, you can configure the CIFS server if you want to create volumes using the CIFS protocol while creating a working environment.
...
Volumes::
The storage volumes can be exposed for use by your applications. You can perform specific workflows to create a volume (using NFS, CIFS or ISCSI protocol), delete a volume, retrieve, or modify an existing volume based on your storage requirements.

Metadata::
The metadata workflows allow you to view and administer the basic configuration of your cloud environment.

Miscellaneous::
A workflow that is not clearly assigned to another functional category is considered miscellaneous. This specific workflow allows you to create a cloud provider account. This account stores and manages your cloud provider credentials securely and creates an identity for users (for example, InstanceProfile for AWS, ManagedIdentity for Azure). This account provides fine-grained access control of services and resources.
