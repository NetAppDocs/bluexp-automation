...
* You can select one or more aggregates depending on your requirement. Perform the workflow link:wf_onprem_ontap_get_aggrs.html[Get aggregates] and choose the `name` value of the aggregates for the `flexGroupVolumeInfo` -> `aggregateList` input parameter.
