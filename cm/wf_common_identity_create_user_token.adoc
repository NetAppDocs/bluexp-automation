---
sidebar: sidebar
permalink: wf_common_identity_create_user_token.html
...
curl example::
[source,curl]
curl --location --request POST 'https://netapp-cloud-account.auth0.com/oauth/token' --header 'Content-Type: application/json' --d @JSONinput --header 'Authorization: Bearer <ACCESS_TOKEN>'
